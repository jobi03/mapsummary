...
class SolutionMapListRight extends React.Component {
  ...
  drop (event, id, vendorNameOld, productName) {
    ...
    return (
      <Panel>
        <Row className='row-cat'>
          <Col xs={8} className='solutionMapList__col-right'>
            {this.props.product &&
              this.props.product.map(pro => (
                <Row key={pro.productName + pro.vendorName}>
                  <Col
                    xs={4}
                    className='logo'
...
export default connect(mapStateToProps)(SolutionMapListRight)
